	...
	while (true) {
		...
		while (isReceviedDataExist()) {
			...
			if (messageHandler->isPacketAvailable(&entry)) {
				if (!this->databaseManager->isTopicExist(entry.PDD_DATA.PARTICIPANT_TOPIC)) {
					...
					databaseManager->InsertEntry(entry.PDD_DATA);
					distributeList = databaseManager->inputQ(entry.PDD_DATA);
			...
			for (it = distributeList.begin(); it != distributeList.end(); ++it) {
				Sleep(100);
				memset(&(ReturnDatagram->PDD_DATA), 0, sizeof(PDD_DATA));
				ReturnDatagram->PDD_DATA.PARTICIPANT_LENGTH_OF_DATA = it->PARTICIPANT_LENGTH_OF_DATA;
				
				messageHandler->addDataToNode(ReturnDatagram, *it);				
				socketManager->sendPacket(entry.PDD_DATA.PARTICIPANT_IP, (const char *)ReturnDatagram, sizeof(_PDD_NODE), DDS_PORT);
			}
