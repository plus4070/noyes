...
namespace CNU_DDS
{
	ByteStream*	SEDPdiscoveredTopicData::Serialize()
	{
		...
		*((short*)(data+data_index))	= PID_PROTOCOL_VERSION.value;
		data_index	+= 2;
		*((short*)(data+data_index))	= sizeof(ProtocolVersion_t);
		data_index	+= 2;
		...
		*((short*)(data+data_index))	= PID_TRANSPORT_PRIORITY.value;
		data_index	+= 2;
		*((short*)(data+data_index))	= sizeof(TransportPriorityQosPolicy);
		data_index	+= 2;
		memcpy(data+data_index, &transport_priority, sizeof(TransportPriorityQosPolicy));
		data_index	+= sizeof(TransportPriorityQosPolicy);

		*((short*)(data+data_index))	= PID_HISTORY.value;
		data_index	+= 2;
		*((short*)(data+data_index))	= sizeof(HistoryQosPolicy);
		data_index	+= 2;
		memcpy(data+data_index, &history, sizeof(HistoryQosPolicy));
		data_index	+= sizeof(HistoryQosPolicy);

		*((short*)(data+data_index))	= PID_RESOURCE_LIMITS.value;
		data_index	+= 2;
		*((short*)(data+data_index))	= sizeof(ResourceLimitsQosPolicy);
		data_index	+= 2;
		memcpy(data+data_index, &resource_limits, sizeof(ResourceLimitsQosPolicy));
		data_index	+= sizeof(ResourceLimitsQosPolicy);
	...
	void		SEDPdiscoveredTopicData::Deserialize(ByteStream* stream)
	{
		...
		if(ordering == recv_ordering)
		{
			do
			{
				...
				if(*parameter_id == PID_TOPIC_NAME.value)
				{
				...
				else if(*parameter_id == PID_TRANSPORT_PRIORITY.value)
				{
					memcpy(&transport_priority, data+data_index, sizeof(TransportPriorityQosPolicy));
				}
				else if(*parameter_id == PID_HISTORY.value)
				{
					memcpy(&history, data+data_index, sizeof(HistoryQosPolicy));
				}
				else if(*parameter_id == PID_RESOURCE_LIMITS.value)
				{
					memcpy(&resource_limits, data+data_index, sizeof(ResourceLimitsQosPolicy));
				}
		...
		else
		{
			do
			{
				...
				{
					memcpy(&transport_priority, data+data_index, sizeof(TransportPriorityQosPolicy));
					ByteStream::ChangeByteOrdering(&transport_priority.value, sizeof(long));
				}
				else if(*parameter_id == PID_HISTORY.value)
				{
					memcpy(&history, data+data_index, sizeof(HistoryQosPolicy));
					ByteStream::ChangeByteOrdering(&history.depth, sizeof(long));
					ByteStream::ChangeByteOrdering(&history.kind, sizeof(HistoryQosPolicyKind));
				}
				else if(*parameter_id == PID_RESOURCE_LIMITS.value)
				{
					memcpy(&resource_limits, data+data_index, sizeof(ResourceLimitsQosPolicy));
					ByteStream::ChangeByteOrdering(&resource_limits.max_instances, sizeof(long));
					ByteStream::ChangeByteOrdering(&resource_limits.max_samples, sizeof(long));
					ByteStream::ChangeByteOrdering(&resource_limits.max_samples_per_instance, sizeof(long));
				}
