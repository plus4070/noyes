...
void TCPSocket::Response() {
	...
	if (RTable->isRequestExist()) {
		...
		if (entry.PDD_HEADER.MESSAGE_TYPE == MESSAGE_TYPE_REQUEST) {
			...
			memcpy(&TE.TN_LEVEL, entry.PDD_DATA.PARTICIPANT_DATA, sizeof(int));

			memcpy(TE.TN_TOPIC, entry.PDD_DATA.PARTICIPANT_TOPIC, MAX_CHAR);
			
			if (TNTable->isEntryExist(TE)) {
				TNTable->getEntry(&TE);
				memcpy(entry.PDD_DATA.PARTICIPANT_DATA, TE.TN_NEXTZONE, sizeof(TE.TN_NEXTZONE));
			...
			cout << "Response ADDRESS :" << entry.PDD_DATA.PARTICIPANT_DATA << endl;
		}
...
static UINT WINAPI sending(LPVOID p) {
	...
	while (1) {
		...
		if (tcpSocket->RTable->isRequestExist()) {
			...
			if (entry.PDD_HEADER.MESSAGE_TYPE == MESSAGE_TYPE_REQUEST) {
				tokenArray = tcpSocket->TNTable->splitTopic(entry.PDD_DATA.PARTICIPANT_TOPIC);
				//수신 메시지 출력
				
				cout << "Request MSG" << endl;
				cout << "Request Topic :" << entry.PDD_DATA.PARTICIPANT_TOPIC << endl;
				cout << "Request TokenLevel :" << entry.PDD_DATA.PARTICIPANT_DATA << endl;
				cout << "Request TYPE :" << entry.PDD_HEADER.MESSAGE_TYPE << endl;

				TE.TN_LEVEL = atoi((const char*)&(entry.PDD_DATA.PARTICIPANT_DATA));
				...
				if (tcpSocket->TNTable->isEntryExist(TE)) {
					...
					memcpy(entry.PDD_DATA.PARTICIPANT_DATA, TE.TN_NEXTZONE, sizeof(TE.TN_NEXTZONE));
					entry.PDD_HEADER.MESSAGE_TYPE = MESSAGE_TYPE_RESPONSE;
					printf("%s", entry.PDD_DATA.PARTICIPANT_DATA);
