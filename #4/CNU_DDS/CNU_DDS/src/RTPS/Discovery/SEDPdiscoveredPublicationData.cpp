...
namespace CNU_DDS
{
	SEDPdiscoveredPublicationData::SEDPdiscoveredPublicationData()
	{
		//memset(this, 0, sizeof(PublicationBuiltinToipcData));
	}

	WriterProxy*	SEDPdiscoveredPublicationData::getWriterProxyInstance()
	{
		unsigned long result	= (unsigned long)this;
		result	+= sizeof(PublicationBuiltinToipcData);

		return (WriterProxy*)result;
	}

	ByteStream*	SEDPdiscoveredPublicationData::Serialize()
	{
		...
		*((short*)(data+data_index))	= sizeof(GUID_t);
		data_index	+= 2;
		memcpy(data+data_index, &remote_writer_guid, sizeof(GUID_t));
		...
		*((short*)(data+data_index))	= PID_OWNERSHIP_STRENGTH.value;
		data_index	+= 2;
		*((short*)(data+data_index))	= sizeof(OwnershipStrengthQosPolicy);
		data_index	+= 2;
		memcpy(data+data_index, &ownership_strength, sizeof(OwnershipStrengthQosPolicy));
		data_index	+= sizeof(OwnershipStrengthQosPolicy);
...
/* for RTI DDS */
/*
		*((short*)(data+data_index))	= (short)0x0053;
		data_index	+= 2;
		*((short*)(data+data_index))	= 4;
		data_index	+= 2;
		data[data_index]	= 0x00;
		data[data_index+1]	= 0x00;
		data[data_index+2]	= 0x03;
		data[data_index+3]	= 0x08;
		data_index	+= 4;
*/
	...
	void		SEDPdiscoveredPublicationData::Deserialize(ByteStream* stream)
	{
		...
		if(ordering == recv_ordering)
		{
			do
			{
				...
				{
					memcpy(&remote_writer_guid, data+data_index, sizeof(GUID_t));
				}
				...
				else if(*parameter_id == PID_OWNERSHIP_STRENGTH.value)
				{
					memcpy(&ownership_strength, data+data_index, sizeof(OwnershipStrengthQosPolicy));
				}
		...
		else
		{
			do
			{
				...
				{
					memcpy(&ownership_strength, data+data_index, sizeof(OwnershipStrengthQosPolicy));
					ByteStream::ChangeByteOrdering(&ownership_strength.value, sizeof(long));
				}
				else if(*parameter_id == PID_LIVELINESS.value)
				{
					memcpy(&liveliness, data+data_index, sizeof(LivelinessQosPolicy));
