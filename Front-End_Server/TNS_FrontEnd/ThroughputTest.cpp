...
ThroughputTest::ThroughputTest(void)
{
}
...
void	ThroughputTest::StreamingTest()
{
...
void	ThroughputTest::StreamingTest_Writer(unsigned long domain_id, unsigned long	num_of_endpoints, unsigned long topic_mode, unsigned long topic_entry, unsigned long num_of_data, unsigned long data_sending_delay)
{
	using namespace CNU_DDS;
	...
	for(i=0; i<num_of_topics; i++)
	{
		...
		sprintf(chr_topic_name, "%s/%s/DDS_%d", "CNU", (i + topic_entry) > 30 ? ((i + topic_entry) > 60 ? "ENG" : "USA") : "KOR", i + topic_entry);
		sprintf(chr_type_name, "%s/%s/DDS_%d", "CNU", (i + topic_entry) > 30 ? ((i + topic_entry) > 60 ? "ENG" : "USA") : "KOR", i + topic_entry);
		printf("%s\n", chr_topic_name);
...
void	ThroughputTest::StreamingTest_Reader(unsigned long domain_id, unsigned long	num_of_endpoints, unsigned long topic_mode, unsigned long topic_entry, unsigned long num_of_data, unsigned long data_sending_delay)
{
	/* Declaration */
	DomainParticipantQos*	part_qos;
	DomainParticipant*		part;
	...
	listener->listener_count	= num_of_endpoints;
	listener->over_count		= 0;
